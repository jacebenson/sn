---
title: "ExpenseManagementUtils"
id: "expensemanagementutils"
---

API Name: global.ExpenseManagementUtils
...
ExpenseManagementUtils.prototype = {
   ...
   getTaxAmount: function (/*Boolean*/salesTax, /*Float*/baseAmount, /*Float*/taxRate){
      ...
      baseAmount = baseAmount.substring( baseAmount.indexOf(';') + 1 ).replace(GlideLocale.get().getGroupingSeparator(), '').replace(decimalSeperator, '.');

      var baseCost = parseFloat(baseAmount, 10);
      var taxCost;
	  var amount;
	  ...
	  taxCost = taxCost.toFixed(2).replace('.', decimalSeperator);
	  amount = amount.toFixed(2).replace('.', decimalSeperator);
   ...
   fetchValues: function(record, table, expenseLine) {
	...
	expenseLineFilter:function(sysId) {
		var filter = "";
		if (sysId) {
			filter += "sys_id!=" + sysId;
		}
