---
title: "UIPolicyBuilder"
id: "uipolicybuilder"
---

API Name: global.UIPolicyBuilder
...
UIPolicyBuilder.prototype = {
	...
	toString: function(ignoreScripts) {
		...
		for (var i = 0; i < this.uiPolicy.fieldPolicies.length; i++) {
			...
			for (var cndx = 0; cndx < fp.list_actions.length; cndx++)
				this._listActionToXML(fpNode, fp.list_actions[cndx]);
		}
	...
	_actionToXML: function(/*xmlnode*/ node, /*action{}*/ action) {
		...
		aNode.setAttribute("cleared", action.cleared);
	},

	_listActionToXML: function(/*xmlnode*/ node, /*action{}*/ action) {
		var xmlUtil = GlideXMLUtil;
		var aNode = xmlUtil.newElement(node, "list_action");
		aNode.setAttribute("name", action.name);
		aNode.setAttribute("visible", action.visible);
		aNode.setAttribute("sys_id", action.sys_id);
	},
	...
	setUIPolicy: function(xmlString) {
		...
		if (list) {
			for (var fpNdx = 0; fpNdx < list.size(); fpNdx++) {
				...
				var children = xmlUtil.getChildrenByTagName(fpNode, "list_action");
				if (children) {
					for (var i = 0; i < children.size(); i++) {
						var action = this._listActionFromXML(children.get(i));
	...
	_actionFromXML: function(/*xmlnode*/ node) {
		...
		action.cleared = this._getAttribute(node, "cleared");
		return action;
	},

	_listActionFromXML: function(/*xmlnode*/ node) {
		var action = {};
		action.name = this._getAttribute(node, "name");
		action.visible = this._getAttribute(node, "visible");
		action.sys_id = this._getAttribute(node, "sys_id");
		return action;
	},
	...
	_process: function(gr, isDataPolicy) {
		gs.addUpgradeDebuggerArtifact("sys_ui_policy", gr.getUniqueValue());
	...
	_processPolicy: function(gr, isDataPolicy) {
		...
		if (isDataPolicy)
			fieldPolicy.source_table = 'sys_data_policy2';
		else
		...
		fieldPolicy.list_actions = list_actions;

		var tableName = 'sys_ui_policy_action';
		var listTableName = 'sys_ui_policy_rl_action';
		...
		//add related list actions
		var listActionGR = new GlideRecord('sys_ui_policy_rl_action');
		listActionGR.addQuery('ui_policy', gr.sys_id);
		listActionGR.query();

		while (listActionGR.next()) {
			var action = {};
			action.name = listActionGR.list.toString();
			action.visible = listActionGR.visible.toString();
			action.sys_id = listActionGR.getUniqueValue();
			list_actions.push(action);
		}
