---
title: "CloudCIReconciler"
id: "cloudcireconciler"
---

API Name: global.CloudCIReconciler
				...
				for(parent in existingCPMap[child]){
					if (parents[parent]) {
						...
						if(!cPMap[child][parent]) {
							gr = new GlideRecord('cmdb_rel_ci');
							gr.get('sys_id', existingCPMap[child][parent]);
							gr.deleteRecord();
						}
					} else {
	...
	updateVmwareGuestStaleness: function(staleVms, updateInstanceStaleness) {
		...
		function updateStaleness(vmInstance) {
			...
			if (vmInstance.sys_id)
				...
				if (!vmInstanceGr.get('sys_id', vmInstance))
					return;
			}

			// Set install status to 'retired'
			vmInstanceGr.install_status = '7';
	...
	*/
	updateStalenessWithField: function(fieldValueMap, tableName, knownChildren, relTypes, parentTypes){
		...
		while(grRel.next()){
			...
			if(grCMDBCI.get('sys_id', parentSysId) && parentTypeMap[grCMDBCI.getRecordClassName()] == 1){
				grRel.deleteRecord();
			}
		}
