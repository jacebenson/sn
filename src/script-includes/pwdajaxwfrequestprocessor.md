---
title: "PwdAjaxWFRequestProcessor"
id: "pwdajaxwfrequestprocessor"
---

API Name: global.PwdAjaxWFRequestProcessor
...
PwdAjaxWFRequestProcessor.prototype = Object.extendsObject(PwdAjaxRequestProcessor, {
    ...
    /**
     * This function starts a master subflow. In case of an exception, will send an error
    ...
    startWorkflow: function() {
        ...
        this._setResponseMessage(res.status, res.message, res.value);
    },

	//To check if the execution is through workflow or flow.
	isFlow: function(){
		var pwdFlowHelper = new PwdFlowHelper();
		var procId = this.getParameter("sysparm_process_id");
		var result =  pwdFlowHelper.useFlow(procId);
		return result;
	},
    ...
    runPostProcessor: function() {
        ...
        var requestId = this.getParameter('sysparam_request_id');
        var workflow_status = this.getParameter('sysparam_workflow_status');
        ...
        if(workflow_status && ( (gr.auto_gen_password || gr.send_email) && gr.sys_id == 'c6b0c20667100200a5a0f3b457415ad5')) {
            gs.getSession().putProperty('pwd_reset_done_redirect', gs.getSession().getProperty('pwd_redirect_url'));
        }

        if (workflow_status) {
            gs.getSession().clearProperty('pwd_redirect_url');
        }
	...
	checkMaximumAttempts: function(){
		var requestId = this.getParameter("sysparam_request_id");
		this._checkMaximumAttempts(requestId);
		this._setResponseMessage("Success", "Checked maximum attempts", "");	
	},
    ...
    _checkMaximumAttempts: function(requestId) {
        var maxAttemptsReached = PwdADMaxAttemptsHelper.checkMaxAttemptsReached(requestId);
        if (maxAttemptsReached) {
            var activity = this.newItem('max_reset_attempts');
            activity.setAttribute('message', gs.getMessage('You have exceeded the maximum number of allowed reset attempt failures'));
            var count = PwdADMaxAttemptsHelper.getMaxResetAttemptsAllowed(requestId);
            var trackingMgr = new SNC.PwdTrackingManager();
            trackingMgr.updateADRequestMaxRetry(requestId, count);
            gs.getSession().clearProperty('pwd_reset_counter');
        }
    },
