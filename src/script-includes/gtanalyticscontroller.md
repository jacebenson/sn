---
title: "GTAnalyticsController"
id: "gtanalyticscontroller"
---

API Name: global.GTAnalyticsController

```js
var GTAnalyticsController = Class.create();
var tourBuilderUtility = new sn_tourbuilder.TourBuilderUtility();
GTAnalyticsController.prototype = {
  ...
  terminalStates: ['completed', 'failed', 'dismissed', 'abandoned'],
  tableNames: {
...
GTAnalyticsController.prototype.addToAnalyticsTable = function(data, currentUser) {
  ...
  gr.matched_roles = tourBuilderUtility.getMatchingRoles(gr.user.toString(), gr.tour.toString()).join(',');
  gr.last_step = 0;
  gr.autolaunch = data.isAutoLaunched;
  gr.insert();
  return gr.getUniqueValue();
};

GTAnalyticsController.prototype.addToAnalyticsEventTable = function(data) {
  if(data.event === this.events.inTransit) return;
...
GTAnalyticsController.prototype.updateAnalyticsTable = function(data) {
  ...
  if (gr.next()) {
  	...
  	if (this.terminalStates.indexOf(lastStatus) > -1 ) {
    	return null;
  	}
  	...
  	} else if (this.terminalStates.indexOf(data.event) && typeof(data.step) !== 'undefined') {
		gr.last_step = escape(data.step);
	}
	if (data.reason) {
		gr.reason = escape(data.reason);
	}
  	gr.update();
  	return gr.getUniqueValue();
  } else {
