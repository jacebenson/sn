---
title: "sc_ic_CategoryRequest"
id: "sc_ic_categoryrequest"
---

API Name: global.sc_ic_CategoryRequest
...
sc_ic_CategoryRequest.prototype = Object.extendsObject(sc_ic_Base,{
	...
	activate: function() {
		...
		if (catGr.get(this._gr[sc_.CATEGORY])) {
			...
			this._gs.addInfoMessage(this._gs.getMessage("Category '{0}' has been activated", catGr.getDisplayValue()));
		}
		else
			this._gs.addErrorMessage(this._gs.getMessage("Category '{0}' has not been activated", this._gr[sc_.TITLE]));
	...
	deactivate: function() {
		...
		if (catGr.get(this._gr[sc_.CATEGORY])) {
			...
			this._gs.addInfoMessage(this._gs.getMessage("Category '{0}' has been deactivated", catGr.getDisplayValue()));
		}
		else
			this._gs.addErrorMessage(this._gs.getMessage("Category '{0}' has not been deactivated", this._gr[sc_.TITLE]));
	...
	createCategory: function() {
		...
		if (catGr) {
			...
			this._gs.addInfoMessage(this._gs.getMessage("Category '{0}' has been created", catGr.getDisplayValue()));
		}
		else
			this._gs.addErrorMessage(this._gs.getMessage("Category '{0}' has not been created", this._gr[sc_.TITLE]));
		this.redirect();
	},
	...
	updateEditorRoles: function(previousGr) {
		...
		var editorsToRemove = au.diff(previousEditorsArr, currentEditorsArr);
		var editorsToAdd = au.diff(currentEditorsArr, previousEditorsArr);
		
		for (var i = 0; i < editorsToRemove.length; i++)
			this.removeEditorRoles(editorsToRemove[i], editorRoles);
		
		for (i = 0; i < editorsToAdd.length; i++)
...
sc_ic_CategoryRequest.createFromCategory = function(category) {
	if (JSUtil.nil(category) || category.getTableName() != sc_.CATEGORY || JSUtil.nil(category.getUniqueValue())) {
		this._gs.addErrorMessage(this._gs.getMessage("Cannot create a Category request from this record"));
		return;
	}
	...
	this._gs.addInfoMessage(this._gs.getMessage("New Category Request record created for category <span style='font-weight:bold'>{0}</span> - <span style='font-style:italic;color:red'>you need to select a Manager for this record</span>", category.getDisplayValue()));
