---
title: "SLATimeLineV2SNC"
id: "slatimelinev2snc"
---

API Name: global.SLATimeLineV2SNC
...
SLATimeLineV2SNC.prototype = {
	...
	getTimeLineDetails: function(taskId /*String - Mandatory*/ , contractSLAIds /*Array*/ ) {
		...
		if (taskId && taskGr.get(taskId)) {
			...
			var taskSLAsCurrentStateInDb = this.slaUtil.grToJsArr(this._getAttachedTaskSLARecs(taskGr, contractSLAIds));
			var contractSLAGrAttachedNSimulated = this._getContractSLARecs(taskSLAsCurrentStateInDb, contractSLAIds, taskGr);

			return this._getTimeLineDetails(taskGr, contractSLAGrAttachedNSimulated,
				taskSLAsCurrentStateInDb);
	...
	_getTimeLineDetails: function(taskGr, contractSLAGrAttachedNSimulated /*Attribute attached and simulated are expected in this object with the respective GRs */ ,
		...
		var taskStatesObj = this._getHistoricalTaskUpdates(taskGr);
		if (!taskStatesObj)
			return this._returnApiError(gs.getMessage('Error in task record history'));
		var taskDeltaChanges = taskStatesObj.taskDeltaChanges;
		var taskGrAtUpdates = taskStatesObj.taskGrAtUpdates;
		if (taskGrAtUpdates.length === 0)
			return this._returnApiError(gs.getMessage('Cannot access task record history'));
		...
		var taskFullSnapAtUpdates = this._prepareTaskFullSnapAtUpdates(taskGrAtUpdates, contractSlas);
	...
	_getTaskRefFieldValueMap: function(contractSlas, taskGr) {
		...
		for (var i = 0; i < contractSlas.length; i++) {
			...
			var j = 0;
			for (j = 0; j < conditionFields.length; j++)
			...
			for (j = 0; j < relatedFieldsForContractSla.length; j++) {
	...
	_prepareTaskFullSnapAtUpdates: function(taskGrAtUpdates, contractSlas) {
		var taskFullSnapAtUpdates = [];
		var allRelatedFields = this._getAllRelatedFields(contractSlas);
		for (var i = 0; i < taskGrAtUpdates.length; i++) {
			var jsObj = this.slaUtil.grToJsObj(taskGrAtUpdates[i]);
			var fullSnap = {};
			// keep only system fields or fields used in conditions
			for (var fieldName in jsObj) {
				if (fieldName.startsWith('sys_') || fieldName.startsWith('variables') || this.arrayUtil.contains(allRelatedFields, fieldName)) {
					// Variables are a special case - if we find any conditions that are for variables include the single "variables" object from the task
					if (fieldName.startsWith('variables.'))
						fullSnap[fieldName] = jsObj["variables"];
					else
						fullSnap[fieldName] = jsObj[fieldName];
				}
			}
			taskFullSnapAtUpdates.push(fullSnap);
		}
	...
	getTaskSLABreakDowns: function(contractSLAGr /*GlideRecord*/ , taskGrAtUpdates /*Array of task GlideRecord*/ ) {
		...
		var startedFromReset;
		var isReset;
		var taskSLADummyGrReset;
		var taskSLACopy;
		var retroactiveStagesObj;
		var taskSLADummyGrResetPseudoUpdate;
			...
			if (taskSLA) {
				...
				if (taskSLADummyGr.getValue(this.ATTR_STAGE)) {
					...
					if (futureStartTime) { //This block is for when SLA has start time later than task update date
						...
						isTerminalStageUpdate = parseInt(taskSLADummyGr.getValue(this.ATTR_ACTIVE)) === 0 || i === taskGrAtUpdates.length - 1;
						if (isTerminalStageUpdate) {
							isReset = taskSLA.isReset;
							...
							startedFromReset = undefined;

							if (isReset) {
								taskDummyGrCurrent = taskGrAtUpdates[i];
								taskSLA = taskSLAController.initNewTaskSLA(taskDummyGrCurrent, contractSLAGr, taskGrAtUpdates);
								if (taskSLA) {
									taskSLADummyGrReset = taskSLA.getGlideRecord();
									...
									if ((i === taskGrAtUpdates.length - 2)) {
										...
										taskDummyGrCurrent = taskGrAtUpdates[i];
										taskSLA = taskSLAController.updateTaskSLA(taskDummyGrCurrent, taskSLA, contractSLAGr);
										taskSLADummyGrResetPseudoUpdate = taskSLA.getGlideRecord();
						...
						if (taskSLAStages.length === 1) {
							...
							if (attachedDate > taskSLADummyGr.start_time.getGlideObject().getDisplayValueInternal()) {
								taskSLACopy = taskSLAController.copyTaskSLA(contractSLAGr, taskDummyGrCurrent, this.slaUtil.copyGlideRecord(taskSLA.taskSLAgr), taskSLA.retroactivePauseDataList);
								retroactiveStagesObj = this._insertRetroactiveStages(taskSLACopy, taskSLAStages[0], attachedDate, taskGrAtUpdates, taskSLAController, contractSLAGr, i, stateChangeOccuredDueToUpdates);
						...
						if (isTerminalStageUpdate) {
							...
							if (taskSLAStages) {
								...
								this._prepareTaskSLA(contractSLAGr, taskSLAStages, stateChangeOccuredDueToUpdates, taskSLAs, attachedDate, taskGrAtUpdates[i], countOfTaskSlas++, null, startedFromReset);
							}
							...
							if (isReset) {
								...
								if (taskSLA) {
									...
									attachedDate = taskDummyGrCurrent.sys_updated_on.getGlideObject().getDisplayValueInternal();
									startedFromReset = {
										taskSLA_text: taskSLAs[taskSLAs.length - 1].text,
										taskSLA_id: taskSLAs[taskSLAs.length - 1].id
									};

									if (attachedDate > taskSLADummyGrReset.start_time.getGlideObject().getDisplayValueInternal()) {
	...
	_insertRetroactiveStages: function(taskSLACopy, attachedStageObj, attachedDate, taskGrAtUpdates, taskSLAController,
		...
		if (attachedStageObj) {
			...
			while (index < taskGrAtUpdates.length - 1 && taskGrAtUpdates[index + 1].sys_updated_on.getGlideObject().getDisplayValueInternal() < startTimeGdt.getDisplayValueInternal())
				index++;
	...
	_getContractSLARecs: function(taskSLAsAttached, contractSLAIds, taskGr) {
		...
		if (simulatedContractSLAIds)
			simulatedContractSLAGr = this._getContractSLARecsByIds(simulatedContractSLAIds, taskGr);

		var attachedContractSLAGr;
		if (attachedContractSLAIds)
			attachedContractSLAGr = this._getContractSLARecsByIds(attachedContractSLAIds, taskGr);
	...
	_getHistoricalTaskUpdates: function(taskGr) {
		var dummyTaskGrList = [];
		var deltaChangesList = [];
		var taskUpdate;
		var walkedRecordGr;
		var hw = this.slaUtil.getHistoryWalker(taskGr.getRecordClassName(), taskGr.getUniqueValue(),
			/*recordLevelSecurity*/ false, /*fieldLevelSecurity*/ false, /*withVariables*/ true, /*walkToFirstUpdate*/ true);
		if (!hw) {
			this.lu.logError("_getHistoricalTaskUpdates: failed getting HistoryWalker to initial update");
			return null;
		}

		do {
			taskUpdate = {};
			walkedRecordGr = hw.getWalkedRecordCopy();
			walkedRecordGr.original_sys_id = taskGr.getUniqueValue();
			dummyTaskGrList.push(walkedRecordGr);
	...
	_addChangedFields: function(walkedRecordGr, taskUpdate) {
		...
		for (var i = 0; i < fields.length; i++) {
			...
			if (fieldName + "" == this.ATTR_SYS_CREATED_ON ||
			    fieldName + "" == this.ATTR_SYS_UPDATED_ON)
				continue;
			...
			if (field.canRead()) {
				...
				if (field.getED().getInternalType() + "" === this.FIELD_TYPE_WF)
