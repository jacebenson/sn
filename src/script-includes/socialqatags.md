...
API Name: sn_kb_social_qa.SocialQATags
...
SocialQATags.prototype = {
	...
	getTags: function(params) {
		...
		var grLabel = new GlideRecord(this.tagTable);
		grLabel.addActiveQuery();
		
		var qc3 = grLabel.addQuery('sys_id', labels);
		if(params.autosuggest && params.query)		
			qc3.addOrCondition('global',true);
		
		if(params.query) 
			grLabel.addQuery('name', 'STARTSWITH', params.query);
		
		//Visibility
		var qc1 = grLabel.addQuery('global', true);
